...
│  ├─ activity.mjs         Reading-events log (started/finished/rated/reviewed)
│  │                       shared by the syncs -> src/data/activity.json.
...
└─ src/
   ...
   │  ├─ activity.json     Reading-events log (generated by the sync scripts).
...
### The updates strip (`src/data/activity.json`)

Goodreads RSS has no event timestamps, so the import/sync scripts detect
"news" by **diffing the freshly-fetched shelves against the JSON already on
disk**: a new currently-reading entry = *started*, a new read-shelf entry =
*finished*, a rating/review appearing on a known book = *rated*/*reviewed*.
Detected events are stamped with the sync time and prepended to
`src/data/activity.json` (committed; pruned after 14 days; deduped across
overlapping runs). The homepage renders events stamped within **24h of the
build** — deploys follow each nightly sync, so in practice "since yesterday's
sync". Guards: a first sync (no prior file) logs nothing, and newly-appearing
reads dated more than two weeks back are treated as backfill, not news.
